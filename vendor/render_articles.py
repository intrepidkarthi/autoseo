...
Vercel with buildCommand: null and outputDirectory: public — nothing runs a build
step — so committing markdown into content/articles/ produces no page at all.
That is exactly what happened to PR #68: merged, and invisible.

autoseo runs this to produce public/blog/<slug>.html and sitemap-articles.xml,
and commits the generated output alongside the markdown. Same renderer, so posts
published this way are byte-identical in structure to the seven already on the
site — same Article/FAQPage/Breadcrumb schema, same styling, same cluster links.

Paths are parameterised (SRC/OUT are set by the caller) since it no longer lives
next to the content it renders. Nothing else is changed; if the site's renderer
is updated, re-copy this file rather than editing it here.
"""
...
SRC = os.environ.get("AUTOSEO_ARTICLES_SRC", "content/articles")
...
def main():
    ...
    sm = os.environ.get("AUTOSEO_SITEMAP_OUT", "public/sitemap-articles.xml")
